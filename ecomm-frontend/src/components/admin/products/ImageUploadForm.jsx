...
const ImageUploadForm = ({ setOpen, product }) => {
  ...
  const onHandleImageChange = (e) => {
    ...
    if (file && ["image/jpeg", "image/jpg", "image/png"].includes(file.type)) {
      ...
      toast.error("Please select a valid Image which (.jpeg, .jpg, .png)");
  ...
  const addNewImageHandler = async (event) => {
    ...
    if (!selectedFile) {
      toast.error("Please selec an image before saving.");
      return;
    }
...
// File inputs are uncontrolled elements

// React does NOT allow directly setting file input value via state
