...
export const increaseCartQuantity =
  ...
  (dispatch, getState) => {
    const { products } = getState().products;
    // products are not loaded directly.
    const getProduct = products?.find(
      (item) => item.productId === data.productId
    );
...
export const getUserCart = () => async (dispatch, getState) => {
  try {
    ...
    dispatch({
      type: "IS_ERROR",
      payload:
        error?.response?.data?.message || "Failed to fetching cart Items",
    });
  }
};
...
export const updateOrderStatusFromDashboard =
  (orderId, orderStatus, toast, setLoader,isAdmin) => async (dispatch, getState) => {
    try {
      setLoader(true);
      const endpoint = isAdmin ? "/admin/orders/" : "/seller/orders/";
      await api.put(`${endpoint}${orderId}/status`, { status: orderStatus });
...
export const updateProductFromDashboard =
  ...
  async (dispatch, getState) => {
    try {
      ...
      toast.error(
        error?.response?.data?.description || "Product udpate failed."
      );
    }
  };
